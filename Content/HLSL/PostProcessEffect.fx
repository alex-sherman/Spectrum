...
float aaThreshold = .7f;
float aaBlurFactor = .35f;
...
float3 Blur(float3 color, float2 texCoord, float blurFactor)
{
    float3 output = (float3)0;
    if (blurFactor == 0) { return color; }
    float weightSum = 0;
    [unroll]
    for (int i = -2; i <= 2; i++) {
        [unroll]
        for (int j = -2; j <= 2; j++) {
            float weight = exp(-(pow(pow(i, 2) + pow(j, 2), 0.5)) / 2 / depthBlurSigma / blurFactor) / (2 * PI * depthBlurSigma * blurFactor);
